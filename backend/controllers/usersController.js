...
const updateUser = async (req, res) => {
    const { id, username, firstName, lastName, about, image, role, active, password } = req.body;
...
const followUser = async (req, res) => {
    ...
    if (!otherUser) {
        return res.status(400).json({ message: 'User to be followed does not exist' });
    }

    if (user.id === otherUser.id) {
        return res.status(409).json({ message: 'Unable to follow own account' });
    }

    if (user.following?.includes(otherUser.id)) {
        return res.status(409).json({ message: `Already follwing ${otherUser.username}` });
    }
...
const unFollowUser = async (req, res) => {
    ...
    if (!otherUser) {
        return res.status(400).json({ message: 'User to be unfollowed does not exist' });
    }

    if (user.id === otherUser.id) {
        return res.status(409).json({ message: 'Unable to unfollow own account' });
    }

    if (!user.following?.includes(otherUser.id)) {
        return res.status(409).json({ message: `Currently not follwing ${otherUser.username}` });
    }
